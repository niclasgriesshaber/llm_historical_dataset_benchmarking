...
Run one or more LLM image-to-text scripts on one or more PDFs, optionally specifying temperature.
Example usage (from within the scripts folder):
    python all_llm_img2txt.py --models gemini-2.0 gpt-4o --pdfs type-1.pdf type-2.pdf type-3.pdf --temperature 0.0

Requirements:
- Expects the individual model scripts to live at ../src/llm_img2txt/<model_name>.py
...
def main():
    parser = argparse.ArgumentParser(
        description="Run specified LLM image-to-text scripts on given PDFs."
    )
    parser.add_argument(
        "--models",
        nargs="+",
    ...
    # Prepare paths
    src_dir = os.path.join(PROJECT_ROOT, "src", "llm_img2txt")
    pdf_dir = os.path.join(PROJECT_ROOT, "data", "pdfs")

    print("=======================================")
    print("Starting all_llm_img2txt.py execution.")
    ...
    print("=======================================")
    print("all_llm_img2txt.py execution finished.")
    print("=======================================")
